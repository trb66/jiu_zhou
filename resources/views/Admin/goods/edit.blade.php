        ...
        <form class="am-form tpl-form-border-form">
                        ...
                        @foreach($types as $v)
                          ...
                          <option 
                            @if($v->id == $goods->cid)
                                selected
                            @endif
                          {{$sum<3?'disabled':''}} value="{{$v->id}}">┟-{{$str}}{{$v->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="am-form-group">
                ...
                <div class="am-u-sm-12">
                    <input type="text" maxlength="30" class="tpl-form-input am-margin-top-xs" id="user-name" placeholder="请输入商品名称" value="{{$goods->name}}">
            ...
            <div class="am-form-group">
                ...
                <div class="am-u-sm-12">
                <input type="number" id="prine" class="am-margin-top-xs" placeholder="输入商品价格" value="{{$goods->price}}">
            ...
            <div class="am-form-group">
                ...
                <div class="am-u-sm-12">
                <input type="text" id="user-weibo" maxlength="30" value="{{$goods->company}}" class="am-margin-top-xs" placeholder="请输入您的商家名">
                <small id="annotation3"></small>
                <div>
                </div>
                </div>
                <input type="hidden" id="iid" data-id="{{$goods->id}}" >
            ...
            <div class="am-form-group">
                ...
                <div class="am-u-sm-12  am-margin-top-xs">
                    <select id='test' data-am-selected="{searchBox: 1}" style="display: none;">
                        <option value="0" @if($goods->status == 0)
                                             selected
                                          @endif 
                                          >在售中</option>
                        <option value="1" @if($goods->status == 1)
                                           selected 
                                          @endif >已下架</option>
                    </select>
                </div>
            ...
            <input type="hidden" id="ccid" data-cid="{{$goods->cid}}" >
            <input type="hidden" id="nname" data-name="{{$goods->name}}" >
            <input type="hidden" id="pprice" data-price="{{$goods->price}}" >
            <input type="hidden" id="ccompany" data-company="{{$goods->company}}" >
            <input type="hidden" id="sstatus" data-status="{{$goods->status}}" >
        </div>
        </div>
    </div>
</div>
...
<script>
           // var cid = $('#ccid').data('id');
           // var name = $('#nname').data('name');
           // var price = $('#pprice').data('price');
           // var company = $('#ccompany').data('company');
           // var status = $('#sstatus').data('status');
           // var data = [cid,name,price,company,status];
      ...
      { 
       ...
       $('#but').click(function(zj){
           ...
           }else if($('#user-weibo').val() == '')
           {
             ...
             var price = $('#prine').val();
             var company = $('#user-weibo').val();
             var status = $('#test option:selected' ).val();
             var id = $('#iid').data('id');
             var datas = [cid,name,price,company,status];
             // let test = {
             //    cid: cid,
             //    name: name,
             //    price: price,
